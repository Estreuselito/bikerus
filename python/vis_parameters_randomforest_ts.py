...
# Split Trainig set once more for an initial visualization of the hyperparameters to avoid overfitting
train_size = 0.8
index = round(len(df) * train_size)
df_2 = df.iloc[:index]
df_2 = df.drop(['datetime'], axis = 1)
X_train_2, Y_train_2, X_test_2, Y_test_2 = train_test_split_ts(df_2, train_size = train_size)
...
# plt.title('Random Forest - TimeSeriesCV - n_estimators')
# plt.legend()
# plt.show()
# plt.savefig("./images/n_estimators_ts", dpi=300)
...
# plt.ylabel('R2 / Pseudo-R2')
# plt.title('Random Forest - TimeSeriesCV - max_depth')
# plt.show()
# plt.legend
# plt.savefig("./images/max_depth_ts", dpi=300)
...
plt.ylabel('R2 / Pseudo-R2')
plt.title('Random Forest - TimeSeriesCV - max_leaf_nodes')
plt.show()
plt.legend()
plt.savefig("./images/max_leaf_nodes_ts", dpi=300)